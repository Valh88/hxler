...
class EntryBuilder {
	...
	static function makeGlueText(elixirModule:String, defs:Array<NifDef>, loadFn:Null<String>, pos:Position):String {
		...
		// The frame struct ships with the package (hxler/core/HxResourceFrame.h,
		// struct hxler::HxResourceFrame) - same layout the Haxe side uses.
		buf.add('static void hx_res_dtor(ErlNifEnv* env, void* obj) {\n');
		buf.add('\thxler::HxResourceFrame* f = (hxler::HxResourceFrame*)obj;\n');
		buf.add('\thx::GCRemoveRoot((hx::Object**)&f->root);\n');
		buf.add('\tf->root = 0;\n');
		buf.add('}\n');
