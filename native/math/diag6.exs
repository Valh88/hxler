defmodule Hxler.Math do
  ...
  def diag_frame(_a), do: :erlang.nif_error(:nif_library_not_loaded)
end

a = Hxler.Math.accum_new()
IO.puts("frame after new        = #{Hxler.Math.diag_frame(a)}")
Hxler.Math.diag_push_local(a, 5)
Hxler.Math.diag_push_local(a, 7)
IO.puts("frame after 2 pushes   = #{Hxler.Math.diag_frame(a)}")
IO.puts("addr(a)                = #{Hxler.Math.diag_addr(a)} (stable across calls?)")
